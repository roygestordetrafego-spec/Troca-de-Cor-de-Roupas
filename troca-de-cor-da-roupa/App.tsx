  ...
  const handleApply = async () => {
    ...
    if (toolMode === ToolMode.VIDEO) {
      ...
      try {
        ...
        setProcessingState({ isLoading: false, statusMessage: '', error: formatErrorMessage(err) });
      }
      return;
    }
    ...
    <div className="flex flex-col h-screen bg-black text-white overflow-hidden font-sans">
      ...
      {!isKeySelected && (
        <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-xl flex items-center justify-center p-6">
          <div className="max-w-md w-full bg-[#0a0a0a] border border-white/10 rounded-[32px] p-10 text-center shadow-2xl">
            <div className="w-20 h-20 bg-[#0ea5e9]/10 rounded-3xl flex items-center justify-center mx-auto mb-8 border border-[#0ea5e9]/20">
              <svg className="w-10 h-10 text-[#0ea5e9]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"></path></svg>
            </div>
      ...
      <header className="h-16 border-b border-[#111] bg-black flex items-center justify-between px-6 shrink-0 z-30">
        ...
        <div className="flex items-center gap-6">
           <div 
             onClick={handleConnectKey}
             className="flex items-center gap-2 bg-[#0a0a0a] px-3.5 py-1.5 rounded-full border border-white/5 cursor-pointer hover:border-[#0ea5e9]/50 transition-colors"
           >
             <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${isKeySelected ? 'bg-emerald-500' : 'bg-red-500'}`}></div>
             <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">
               {isKeySelected ? '4K MODE ACTIVE' : 'CONNECT API KEY'}
             </span>
           </div>
